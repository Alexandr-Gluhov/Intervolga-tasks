...
TXT;
// поиск по шаблону вида разделитель(ли) - тег(и) - слово(число, дата) повторяющемуся 29 раз
preg_match("/([^а-яёa-z\d<]*((<(\/?[^>]+)>)[^а-яёa-z\d<]*)*([а-яёa-z]+|(\d-[а-яёa-z])|\d{2}\.\d{2}\.\d{4}|\d+)){29}/ui", $text, $out);
echo $out[0] . '…';
